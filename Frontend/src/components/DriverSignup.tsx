...
export function DriverSignup({ onBack, onSuccess }: DriverSignupProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Call backend driver signup API
      const data = await apiClient.post('/driver/signup', {
        name: formData.name,
      ...
      >
        ...
        <Card className="bg-slate-800/50 backdrop-blur border-slate-600 p-8">
          <div className="flex flex-col items-center text-center space-y-6 mb-8">
            ...
            <div>
              <h2 className="text-white mb-2">Driver Sign Up</h2>
              <p className="text-slate-300">Join our driver community today</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              <Label htmlFor="license_no" className="text-slate-200">License Number</Label>
              <Input
                id="license_no"
                type="text"
